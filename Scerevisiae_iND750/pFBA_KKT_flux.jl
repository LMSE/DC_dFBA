function pFBA_KKT_flux(c0,v0)
 ...
 for i in 1:nc
  ...
  vo[i=1:nfe], vo_max*((uk[1,i])/(Ko+(uk[1,i])) )
...
#@objective(m, Min, 1)
  ...
  @constraints(m, begin
      ...
      m2[i=1:nfe, j=1:ncp], cdot[2,i,j] == (uk[2,i]*Gf + vs[glu]*v[glu,i]*c[1,i,j]*cs[1])/cs[2]
      m3[i=1:nfe, j=1:ncp], cdot[3,i,j] == (vs[eth]*v[eth,i]*c[1,i,j]*cs[1])/cs[3]
      m4[i=1:nfe, j=1:ncp], cdot[4,i,j] == uk[2,i]/cs[4]
      ...
      #c_LB_V[i=1:nfe, j=1:ncp], -c[4,i,j]  <= -0.5
